 ...
 *
 * @Input() maskType {string} [maskType=""]
 ...
 *
 * For Time correction example:
 * Use [regexp]="'^[0-2]{1}$|^[0-2]{1}[0-9]{1}$|^[0-2]{1}[0-9]{1}[0-5]{1}$|^[0-2]{1}[0-9]{1}[0-5]{1}[0-9]{1}$'"
 * This will be check for HH:mm format
 *
 * For Date correction example:
...
export class MaskDirective implements OnInit {
  ...
  @Input() private conditions: { cases: { position: number, regexp: string }[], placeholder: string }[] = [];
  ...
  set maxLength(value: number) {
    const maxLength = this.elementRef.nativeElement.maxLength ? this.elementRef.nativeElement.maxLength : this._placeholder.length;
