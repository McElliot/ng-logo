      ...
      if (typeof val !== 'undefined' && val !== null && val.constructor !== Function) {
        if (val && val.constructor === Object) {
          ...
          if (val.constructor === Boolean) {
            ...
            this.dom = this.dom.ele('ss:Cell').att('ss:Index', columnIndex).ele('ss:Data').att('ss:Type', 'Number').txt(val).up().up();
          } else if (val !== undefined && val !== null) {
            const str = val.split('\u000b').join(' ');
            // chr = str.match(chars);
            this.dom = this.dom.ele('ss:Cell').att('ss:Index', columnIndex).ele('ss:Data').att('ss:Type', 'String').txt(str).up().up();
          }
        }
      }
    });
